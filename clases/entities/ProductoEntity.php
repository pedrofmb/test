...
class ProductoEntity extends EntityBase implements DBOCrud{
    ...
    public function storeFormValues(&$options) {
        ...
        if (isset($options["producto_id"])) {
            if ($options["producto_id"] == "") {
                $options["producto_id"] = null;
            }
        }
    ...
    public static function getById($producto_id) {
        try{
            global $pdo;
            $sql = "SELECT * FROM producto WHERE producto_id=:producto_id LIMIT 1";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':producto_id', $producto_id, PDO::PARAM_INT);
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if($row){
                return new ProductoEntity($row);
            }else{
                return false;
            }
        } catch (Exception $exc) {
    ...
    public static function getByFields($whereParams = array(), $orderParams = array(), $start = 0, $limit = LIMIT_RESULT) {
        try {
            ...
            $query = 'SELECT SQL_CALC_FOUND_ROWS * FROM producto ' . $whereClause . $orderClause .' LIMIT :start, :limit';
            ...
            while($row = $stmt->fetch()){
                $producto = new ProductoEntity($row);
                $productos[] = $producto;
            }
            return array("productos"=>$productos, "totalCount"=>$rowTotal["totalCount"]);
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    }
    ...
    public function delete() {
        try {
            global $pdo;
            $stmt = $pdo->prepare('DELETE FROM producto WHERE producto_id=:producto_id LIMIT 1');
    ...
    public function insert() {
        try {
            ...
            $stmt->bindParam(':producto_nombre', $this->producto_nombre, PDO::PARAM_STR);
            $stmt->bindParam(':producto_fecharegistro', $this->producto_fecharegistro, PDO::PARAM_STR);
    ...
    public function update() {
        try {
            ...
            $stmt = $pdo->prepare(
                    ...
                    producto_fecharegistro=:producto_fecharegistro,
